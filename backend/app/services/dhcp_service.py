...
class DhcpService:
    ...
    def __init__(self, app_settings: AppSettings, mac_vendor_service: MacVendorService) -> None:
        self.config_file_path = app_settings.dnsmasq_config_file_path
        self.root_path = app_settings.root_path
        ...
        # Main lease cache
        self._lease_cache: list[DhcpLease] | None = None
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        return os.path.join(self.root_path, path.lstrip("/")) if self.root_path else path
    ...
    def _parse_dhcp_range_line(self, line: str) -> DhcpPool | None:
    ...
    def _parse_lease_duration(self, duration_str: str) -> int | None:
    ...
    def parse_lease_file(self) -> list[DhcpLease]:
        ...
        Format per line: <expiration_timestamp> <mac_address> <ip_address> <hostname> <client_id>
        Timestamp of 0 indicates a static lease that never expires.
        """
        leases = []

        resolved_lease_path = self._resolve_path(self.lease_file_path)  # type: ignore[arg-type]
        with open(resolved_lease_path, encoding="utf-8") as file:
    ...
    def _parse_lease_line(self, line: str) -> DhcpLease | None:
    ...
    def _get_pool_for_ip(self, ip_address: str) -> str | None:
    ...
    def _validate_ip_address(self, ip_address: str) -> None:
        ...
        for part in parts:
            try:
                ...
                raise ValueError(f"Invalid IP address format: {ip_address}") from None
    ...
    def _read_leases_from_disk(self) -> list[DhcpLease]:
        ...
        resolved_lease_path = self._resolve_path(self.lease_file_path)
