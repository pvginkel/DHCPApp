...
class Environment(BaseSettings):
    ...
    CORS_ORIGINS: list[str] = Field(
        default=["http://localhost:3300"], description="Allowed CORS origins"
    )
    ...
    FRONTEND_VERSION_URL: str = Field(
        default="http://localhost:3300/version.json",
...
class Settings(BaseModel):
    ...
    debug: bool = True
    cors_origins: list[str] = Field(default=["http://localhost:3300"])
    ...
    baseurl: str = "http://localhost:3300"
    ...
    frontend_version_url: str = "http://localhost:3300/version.json"
    sse_heartbeat_interval: int = 5  # Resolved: 30 for production via load()
    sse_gateway_url: str = "http://localhost:3302"
